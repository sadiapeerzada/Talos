"""
A genuinely LLM-backed `Brain` implementation, hitting the Groq API
(https://console.groq.com) instead of the deterministic rule-based fixture
in brain.py.
...
GroqBrain is a real, independent tool-using agent: its own system prompt,
...
own fixtures" proof point.

Groq's free tier is used here (OpenAI-compatible /chat/completions with
tool-calling support) -- get a free key at https://console.groq.com/keys
and set it as the GROQ_API_KEY environment variable before running
...
class GroqBrain(Brain):
    """Real LLM-backed brain using Groq's OpenAI-compatible tool-calling API.
    ...
    def __init__(
        ...
        if not api_key:
            raise RuntimeError(
                "GROQ_API_KEY is not set. Get a free Groq API key at "
                "https://console.groq.com/keys and run:\n"
        ...
        self._tool_names = {t["function"]["name"] for t in self._tool_schema}
    ...
    def decide(
        ...
        if tool_calls:
            ...
            return Decision(
                ...
                tool_name=fn["name"],
                tool_args=args,
                debug_reason=f"groq/{self._model} chose tool {fn['name']}",
            )

        return Decision(
            action="final",
            text=choice.get("content") or "",
            debug_reason=f"groq/{self._model} gave a final answer",
        )
